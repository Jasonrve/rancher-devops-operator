...
public sealed class McpServer
{
    ...
    public McpServer(IMcpToolCatalog catalog, IMcpToolExecutor executor, IMcpTokenStore tokenStore, ILogger<McpServer> logger)
    {
        _catalog = catalog;
        _executor = executor;
    ...
    public async Task<IResult> HandleAsync(HttpContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            principal = await ResolvePrincipalAsync(context, cancellationToken);
        }
    ...
    private async Task<McpPrincipal> ResolvePrincipalAsync(HttpContext context, CancellationToken cancellationToken)
    {
        var authHeader = context.Request.Headers.Authorization.ToString();
        if (string.IsNullOrWhiteSpace(authHeader))
        {
            return McpPrincipal.AnonymousViewer();
        }
        ...
        var record = await _tokenStore.ResolveAsync(rawToken, cancellationToken);
        if (record is null)
        {
            throw new UnauthorizedAccessException("Invalid MCP token");
        }

        return new McpPrincipal(record.Role, false, record.SecretName, record.TokenHash);
    }
