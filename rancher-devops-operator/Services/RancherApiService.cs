...
public class RancherApiService : IRancherApiService
{
    ...
    public async Task<RancherPrincipal?> GetPrincipalByNameAsync(string principalName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var payload = new RancherPrincipalSearchRequest
            {
                Name = principalName,
                PrincipalType = null
            };

            var json = JsonSerializer.Serialize(payload, RancherJsonSerializerContext.Default.RancherPrincipalSearchRequest);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
